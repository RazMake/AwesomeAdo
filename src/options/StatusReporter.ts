 ...
 * visible to the user (and to screen readers), while the injected logger records the full detail to
 * the device-local diagnostics log (also mirrored to the console).
 */
export class StatusReporter {
  ...
  /**
   * Show an error to the user and record the full error for diagnostics. Defined as a bound field so
   * it can be handed directly to consumers that expect a plain `(error) => void` callback.
   */
  readonly report = (error: unknown): void => {
    this.logger.error("Options page error", error);
