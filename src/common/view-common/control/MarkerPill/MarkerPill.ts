 ...
 *
 * ONE control for every place a marker is shown — inside the right-click command that applies it and
 * on the board's filter row — because those two surfaces have to be recognizably the same thing: the
 * pill in the menu is a preview of the pill the item will wear. Two separate renderings would drift
 * in precisely the detail (the color) that carries the whole meaning.
 */
export function renderMarkerPill(doc: Document, options: MarkerPillOptions): HTMLElement {
  ...
  if (interactive) {
    ...
    pill.addEventListener("click", () => options.onToggle?.());
  }
