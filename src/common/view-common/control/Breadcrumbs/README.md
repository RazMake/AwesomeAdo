...
Builds a themed `<nav>` of anchor segments, or returns `null` when `options.segments` is empty so the
caller can omit the row entirely rather than mount an empty landmark.
...
| `separator` | The glyph shown between segments. Defaults to a backslash (`\`).     |
