...
A strictly-sequential queue for single work-item field writes back to Azure DevOps (`System.State`,
a type's ETA date field, or any other field reference name).

## Why

ADO writes are latency-bound and rule-driven. Firing them concurrently would race on `System.Rev`
and let a stale rev clobber a fresh one. `FieldWriteQueue` serializes every write so each observes
...
```typescript
import { FieldWriteQueue } from "path/to/FieldWriteQueue";

// writeField is the injected IWorkItemFieldWriter-style function; logger is source-scoped.
const queue = new FieldWriteQueue(services.writeField, services.logger);
...
- **`new FieldWriteQueue(writeField, logger)`** — construct one queue per view instance (per tab).
