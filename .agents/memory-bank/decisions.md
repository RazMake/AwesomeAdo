...
- Decision: Every UI control an enhanced view renders (badges, pills, buttons, twisties, dropdowns,
  ...
  endpoints and framing supplied by the theme. Direct work-item type colors retain ADO's hue on
  light schemes and mix it toward the active foreground on dark schemes; controls that derive their
  own tint receive the normalized raw color instead. Semantic status colors remain muted/discrete,
  and reusable controls remain under `view-common/control`.
